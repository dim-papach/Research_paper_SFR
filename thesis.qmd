...
### Hubble-de Vaucouleurs Classification
...
[^2]:
    -   [@calzettiStarFormationRate2012], [@ASTR620GalaxiesFall]
...
According to [@madau2014], this method often underestimates the SFR, since different galaxy populations may systematically follow distinct absorption mechanisms depending on their characteristics.
...
gnuplot -e "
set terminal svg;
set output 'figure/combined_models.svg';
set xlabel 'Time (t)';
set ylabel 'SFR(t)';
set title 'Star Formation Rate Models';
set grid;
plot exp(-x/8) title 'Exponential Decline (tau=2)' with lines lw 2, \
     x*exp(-x/2) title 'Delayed Exponential (tau=2)' with lines lw 2, \
     (1/1)*exp(-((log(x)-2)**2)/(2*1**2)) title 'Log-Normal (tau=1, T0=2)' with lines lw 2, \
     1 / ((x/2)**2 + (x/2)**3) title 'Double Power Law (alpha=2, beta=3)' with lines lw 2;
"
```
...
data <- data.frame(
  ...
  SFR = c(
    ...
    log_normal(t, tau = 2, T0 = 12),
    double_power_law(t, tau = 2, alpha = 2, beta = 8)
  ),
  Model = rep(c("Exponential Decline", "Delayed Exponential", "Log-Normal", "Double Power Law"), each = length(t))
)
...
ggplot(data, aes(x = t, y = SFR, color = Model)) +
  ...
  labs(
    ...
    x = expression("Star Formation time (" * t[sf] * ") [Gyr]"),
    y = expression("SFR(" * t[sf] * ") ["* M['☉']/yr *"]" ),
    color = "Model"
  ) +
...
The Lilly-Madau plot is one of the most important plots in the field of galaxy evolution. It describes how the SFRD of the universe evolved over time, with observational data. to figure out
...
$V = H_0\times d$, where $H_0 \approx 69.8 \text{ km/s/Mpc}$ is the Hubble constant and $d$ is the distance between the two galaxies.[^3]

[^3]: We also have non-Hubble motions $V = H_0\times d + V_0$, where $V_0$ is the peculiar velocity and it could, for example, be due to galaxy cluster dynamics. For the current explanation we are going to ignore it. This way the radial velocity $v$ is equal to $V$
