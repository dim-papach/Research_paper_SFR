---
  ...
  author: 
  - name: Dimitrios Papachristopoulos
    role: sdf
    affiliations:
      - id: undgdt
        department: ID number 1110 2018 00142 
        name: Undergraduate student
  - name: Despina Hatzidimitriou 
    affiliations:
      - id: sprvsr
        name: Supervising professor 
        department: National and Kapodistrian University of Athens
  fontfamily: fontenc
  mainfont: "Times New Roman"
  format: 
    titlepage-pdf: 
      titlepage: vline
      titlepage-logo: "template/uoa_new_logo_red.png"
...
The Star Formation History (SFH) of a galaxy can offer many insights not only for the evolution and the future of the galaxy, but also for the evolution of the Universe. This is why there are various theoretical models trying to describe the SFH of galaxies. One of those models is the Delayed-Tau model, which approximates the star formation of galaxies as a function with a rising Star Formation Rate (SFR) at the beginning, until it reaches a peak at a time $\tau$, unique for each galaxy, and then it drops at an exponential rate.

@haslbauerCosmologicalStarFormation2023 argue that the use of Delayed-$\tau$ model for the galaxies of the Local Cosmological Volume (LCV) is opposed to the Lilly-Madau plot ([@madauCosmicStarFormation2014]), which plots the observed SFR's of galaxies with the corresponding redshifts ($z$) and calculates a cosmic SFR peak at $z\approx 2$. The way they calculated this inconsistency is by using observational data for SFR and galaxy masses from the UNGC catalog (@karachentsevUPDATEDNEARBYGALAXY2013, @karachentsevSTARFORMATIONPROPERTIES2013) for calculating the parameters (the timescale $\tau$ and the normalization constant $A_{del}$) of the model. This calculation for the galaxies of the Local Cosmological Volume (LV), allows the investigation of the SFR throughout the life of each galaxy and so we can find the expected time of peak of the SFR.
...
**Keywords:** Galaxies, Galaxy Evolution, Star Formation History (SFH), Star Formation Rate (SFR), Delayed-$\tau$, Local Cosmological Volume, Lilly-Madau Plot, Redshift, Newton-Ramphson, Markov Chain Monte Carlo (MCMC).

## Greek

Οι γαλαξίες είναι τεράστια συστήματα από αστέρια, πλανήτες, αέρια και σκόνη που συγκρατούνται μαζί λόγω της βαρύτητας. Κάθε γαλαξίας αποτελεί ένα «κοσμικό νησί» με δισεκατομμύρια ή ακόμα και τρισεκατομμύρια αστέρια, πλανήτες, μαύρες τρύπες και άλλα αστρονομικά αντικείμενα. Κάθε γαλαξίας διαφέρει τόσο στο πόσο μαζικός είναι όσο και στην μορφολογία του. Συνεχώς, εντώς των γαλαξιών, νεφελώματα τα οποία αποτελούνται από αέρια και σκόνη, καταρρέουν λόγω της βαρύτητας τους και παράγουν αστέρια. Η παραγωγή των άστρων επιρρεάζεται από τα χαρακτηριστικά του εκάστοτε γαλαξία και μέσω της μελέτης της ιστορίας της αστρογέννεσης των γαλαξιών, μπορούμε να καταλάβουμε την προέλευση των αστρικών και πλανητικών συστημάτων αλλά και την εξέλιξη τόσο των γαλαξιών όσο και του σύμπαντος.

Μέσω φασματοσκοπικών και φωτομετρικών μεθόδων μπορούμε να υπολογίσουμε των ρυθμό αστρογέννεσης (Star Formation Rate, SFR) των γαλαξίων σε διάφορες ηλικίες του γαλαξία (διαφορετικά φάσματα που χρησιμοποιούνται για τον υπολογισμό διαφορετικών αστρικών πληθυσμών), και με αυτών των τρόπο είναι δυνατόν να μάθουμε την ιστορία αστρογένεσης του κάθε γαλαξία (Star Formation History, SFH). Κάποια μοντέλα προσπαθούν να παραμετροποιήσουν την SFH, χρησιμοποιούντας συναρτήσεις της μορφής $\text{SFR} = f(t)$, προσπαθώντας να προσεγγίσουν την πραγματική SFH με μία ομαλή και εξιδινακευμένη εξίσωση που αγνοεί την ποικιλομορφία των γαλαξιών.

{{<pagebreak>}}

# Galaxies and Galaxy Morphology
...
[^1]: For more information about the Great Debate, visit <https://apod.nasa.gov/debate/1920/cs_real.html>
...
-   **Late-type galaxies** (Spirals and Irregulars):
    ...
    -   Assigned positive Numerical Morphological T-Types, extending to +11 for extreme irregulars.
...
# Star Formation
ing professor
The SFH of a galaxy describesution of its star formation rate over time. By selecting an appropriate model for SFH, we can analyze stellar production, predict periods of active or quiescent star formation, and determine when SFR stabilizes.
...
$$
\int_{t_\text{start}}^{t_\text{final}} \text{SFR}(t)dt = \zeta M_*(t_{sf}),\ t_{sf}=\text{Time of Star Formation,} 
$$ {#eq-sfr_int}
...
-   **H**\boldmath$\alpha$ **Emission**: Young, hot, massive stars (O-type stars, \~10 Myr, \~20 $M_\odot$) produce a number of ionizing photons, which they ionize the surrounding hydrogen rich gas. The hydrogen undergoes recombination cascades which produce Balmer emission lines of $H\alpha$ (0.6563 $\mu m$) and $H\beta$ (0.4861 $\mu m$). Dust can significantly affect observations. Since this estimator depends on young stars, it gives us information about the present day star formation (\~10 Myr).

-   **Far-Ultraviolet (FUV) Flux**: Mainly emitted by young, hot stars ( B-type stars, \~100 Myr). Dust presence can also significantly affect observations. Since this estimator depends on young stars, it gives us information about the present day star formation (\~100 Myr).

-   **Infrared (IR) Flux**: The stars in a galaxy can heat up the dust in different ways, which then emits radiation in different parts of the IR spectrum. For example, young and massive, short-lived stars, emit UV radiation which then the heated dust emits in a wavelength of $\approx 60 \mu m$, whereas dust heated by UV-faint old or low-mass stars will emit at $\approx 100-150\mu m$. As a result, the total IR emission is age-agnostic and provides a more accurate approximation of the SFR because it accounts for contributions from both young and old stellar populations. Since this estimator is somewhat age agnostic, but is derived from the radiaton of heated dust, it is an estimator for the current star formation rate and not the present SFR (\~ 300 Myr).
...
where $\mathcal{K}_i$ is a constant specific to each $L_i$ ($i =$ H$\alpha$, IR, radio, FUV, X).

![Plot showing the linear relation $\log\text{SFR}_{FUV} = \log\text{SFR}_{H\alpha}$, as well as their distributions, based on the data from UNGC](figure/log_SFR_FUV_Ha.png){#fig-SFR-Ha-FUV fig-align="center" width="400" fig-pos="t"}
...
![The Main Sequence of the galaxies within the radius of 10 Mpc, with the colorbar representing their morphological type. From the linear fit we get $\alpha(T_0) = 0.86$ and $\beta(T_0)= 9.83$. Early types have lower Stellar Masses and SFR, while spiral galaxies have more intense SF](method_comparison/ms_plots/ms_morph.png){#fig-ms-Type width="60%"}
...
where $\tau$ is the timescale, $t_{\text{sf}} = T_0 - t_\text{start}$ is the star formation time, $T_0$ is the age of the Universe, and $t_\text{start}$ is the time when star formation began.
...
![Star Formation Rate over the time of star formation, for different parametric models](images/sfr-models.png){fig-align="center" width="500" fig-pos="ht"}
...
![The history of cosmic star formation as shown in @madauCosmicStarFormation2014](images/lily-madau.png){fig-align="center" width="450" fig-pos="t"}
...
$$
\text{SFR}_0 = \text{SFR}(t_{sf}) = A_{del}\frac{t_{sf}}{\tau^2}e^{-t_{sf}/\tau},\ t_{sf} = T_0-t_\text{start}
$$ {#eq-delayed-current}

![The evolution of the SFR of a galaxy over the time of star formation, according to the delayed-$\tau$. The time need for the galaxy to reach the peak of its star formation is the timescale $\tau$ of the model](images/delayed_tau_sfr.png)

where the current $\text{SFR}_0$ of the galaxies, at the current age of the universe $T_0=13.8$ Gyr, is given in the catalogs. According to these model the timescale, at which the SFR peaks, is around $3.5\lessapprox\tau\lessapprox 4.5$ [@speagleHighlyConsistentFramework2014]. If we intergrate the SFR(t), we can calculate the present day averaged SFR:
...
$$
\overline{\text{SFR}_\text{del}} = \frac{\zeta M_*}{t_{sf}}
$$ {#eq-averaged-SFR-M}
...
The specific-SFR (sSFR) provides a normalized view of star formation intensity. Assuming a function $g(x)$ and its derivative $g'(x)$ for the Newton-Raphson algorithm:
...
![Paramater distribution of the delayed-$\tau$ SFH for the galaxies of the LCV. The colorbar represents the present-day stellar mass of the galaxies. The left plot shows the whole parameter range of the model, including negative timescales and $A_{del}$ values that occur by instabilities. In the right plot, the region $\tau \in [0,16]$ and $A_{del}\in[0,10^{13}]$ is depicted. The majority of the galaxies have a timescale of about 3.5-4.5 Gyr as expected from @speagleHighlyConsistentFramework2014](NR/tau_A_double_plot.png){#fig-A-tau}
...
Since convergence can not occur for $x\ll 1$, this method should not be used for galaxies in the begining phases of their star formation [^10]. If the assumption, $t_{sf}=13.6$ Gyr for all the galaxies is correct, then an inconstancy in the plot $A_{del}-\text{sSFR}$ should only be observed, for higher values of the sSFR, since, according to the delayed- $\tau$ model, higher sSFR values indicate a rising SFR, while lower SFR values indicate that the bulk of $M_*$ is already produced and the galaxy is in the declining SFR phase. However, as seen from @fig-sSFR-A, this inconsistency occurs in galaxies for both high and low sSFR's, meaning that the the model has convergence issues both for slow rising SFR and quenching galaxies, indicating an intrinsic problem with this approach.
...
![$A_{del}-\text{sSFR}$ plot, where $A_{del}$ is calculated with different methods.](method_comparison/sSFR_A.png){#fig-sSFR-A}
...
we can create a MCMC model to calculate the parameters, with the priors $t_{sf}\sim \text{uniform(lower bound = 1 Gyr, upper bound = 13.8 Gyr)}$, $\zeta \sim \text{normal(mean=1.3, }\sigma = 0.01)$, within the limits $[1,2]$ and the likelihood $\log(\text{SFR}_{obs}) \sim \text{normal}\left(\log(\text{SFR}_\text{MCMC}), \sigma = 0.1\right)$, while taking the Stellar Masses of the galaxies as they are, without any variance, and $A_{del}$ is calculated by @eq-A. For the $\tau$ we will consider two different priors, $\tau \sim \text{uniform(lower bound = 1 Gyr, upper bound = 13.8 Gyr)}$ and $\tau \sim \text{normal(mean = 4 Gyr, }\sigma  = 13.8 \text{Gyr)}$. The MCMC run consists of 6 chains with 8000 iterations each (2500 iterations for the burn-in phase). Using this method, the galaxies can have different $t_{sf}$, while only taking positive values for their timescales.

The prior of the $\tau$ and $t_{sf}$ follow a uniform distribution, since we do not want to introduce any unwanted bias in the value selection of the MCMC sampler. However, there is bibliography that could justify using a normal distribution for $\tau$, for example @speagleHighlyConsistentFramework2014 and @kroupaConstraintsStarFormation2020 explain that the galaxies of the Main Sequence are expected to have $3.5\lesssim\tau/\text{Gyr}\lesssim4.5$. The same prior will not apply for $t_{sf}$ since it could significantly effect the results, especially of younger galaxies.

[^11]: For more information about the usage of Stan and its metrics can be found here: <https://mc-stan.org/>

::: {#fig-distr-comparison layout-ncol="2" fig-pos="h"}
![Distribution of $\tau$ values.](method_comparison/tau_hist.png){#fig-tau-hist-both}

![Distribution of $t_{sf}$ values.](method_comparison/tsf_hist.png){#fig-tsf-hist-both}

Distribution of the results of $\tau$ and $t_{sf}$ from all the methods.
:::

Using these priors, the metrics of the Stan software[^11] give good results ($\hat{R}<1.001$ and $\text{ESS} >1000$), while the calculated values correspond to the data of the catalog. Specifically, we use the data to recalculate the SFR from the predicted parameters, using the delayed-$\tau$ model and compare the normalization constant to the stellar masses of the galaxies (@fig-A-M-mcmc).

In @fig-sSFR-A, it can be seen that the MCMC run does not have the same inconsistencies with the N-R method, and @fig-tau-hist-both shows that the MCMC method captures better both the galaxies with small timescales, and the galaxies with slow rising SFR's.

![Main Sequence of of the galaxies, color coded by the timescale $\tau$, from the MCMC run from with the uniform prior for $\tau$. The galaxies with slow rising SFR's are the galaxies above the center of the MS, while galaxies with $\tau$ predicted from @speagleHighlyConsistentFramework2014 are in the lower part of the MS. In the center of the MS are the rest of the galaxies](method_comparison/ms_plots/ms_tau_up.png){#fig-MS-tau width="60%" fig-pos="h"}

In @fig-tau-hist-both, the distribution of the $\tau$ from the MCMC run with the uniform prior, has two peaks, the first peak at $\sim 2$ Gyr and one at $\sim 12$ Gyr. As shown in @fig-MS-tau, the galaxies contributing in the two peaks are in different parts of the Main Sequence, while the MCMC run with the normal prior depicts the galaxies of the MS as they are predicted by @speagleHighlyConsistentFramework2014, since every galaxy reaches its peak SFR at the a timescale $\tau < 5$. Additionaly the MCMC run with the normal prior, predicts a peak of the $t_{sf}$ distribution around 12-12.5 Gyr, closer to assumed values in @kroupaConstraintsStarFormation2020 and @haslbauerCosmologicalStarFormation2023 (@fig-tsf-hist-both).

::: {#fig-A-M-mcmc layout="[[50,50],[1]]" fig-pos="h"}
![Uniform $\tau$ prior](method_comparison/A_M_MCMC_uni.png){#fig-A-M-mcmc-uni}

![Normal $\tau$ prior](method_comparison/A_M_MCMC_norm.png){#fig-A-M-mcmc-norm}

![Newton-Raphson Method](/NR/A-M_*-c_x.png){#fig-A-M width="60%"}

Linear regression for $A_{del}-M_*$ for the $A_{del}$ values from the two MCMC runs and the Newton-Raphson method, with the colorbar representing the ratio $x=t_{sf}/\tau$. For the MCMC runs, the black dashed line represents the regression weighted by $x$. For the upper left plot, the regression yields a correlation of 94%, while the weighted regression is the fit $\log(A_{del}) = 0.95\cdot\log(M_*)+0.60$, with a correlation of 98%. For the upper right plot, the correlation is higher, at 99%, with a weighted regression of $\log(A_{del}) = 0.97\cdot\log(M_*)+0.44$, with a correlation of 99.5%. For the N-R plot, we do two different regressions for the two distinct regions of $A_{del}$, with the upper region only having a correlation of 55%, while the lower region closely relating the normalization constant with the Stellar Masses of the galaxies, as described in the MCMC plots.
:::


Having calculated the model parameters, it is possible to reconstruct the SFH of the galaxies, and from the @eq-SFRD, it is possible to calculate the SFRD of the LCV and recreate the Lilly-Madau Plot. Using the equations @eq-delayed-current and @eq-SFRD, we get the equation:

$$
\text{SFRD(T)} = \frac{1}{V}\sum^N_{i=1} A_{del}\frac{T-t_\text{start}}{\tau^2}\exp\left(-\frac{T-t_\text{start}}{\tau}\right),
$$ {#eq-SFRD-delayed}

where $t_\text{start}$, is calculated by the computed $t_{sf}=T_0-t_\text{start}$, and $T$ is the age of the universe ($T_0$ is the current age=13.8 Gyr). Using this equation we can recreate the parameterized SFRD.

<!-- ![Residuals of the SFH reconstruction of the LCV based on the theoritical line given by @madauCosmicStarFormation2014](method_comparison/sfrd_residuals_vs_lm.png) -->

::: {#fig-lilly-madau-rec layout-ncol="2"}
![SFH Reconstruction of the Local Cosmological Volume](method_comparison/sfrd_comparison_custom_axes_log_values.png)

![Ratio between the cosmic SFRD measured by @madauCosmicStarFormation2014 and the SFRD of the LCV](method_comparison/sfrd_ratio_vs_lm.png)

SFH Reconstructions of the Local Cosmological Volume, based on the three different methods. Ignore the Skew prior (Red), is a proof of concept.
:::

As seen from @fig-lilly-madau-rec, the MCMC method can approximate better the SFRD of the LCV up until $z\sim 2$ with a factor less than 2. At the peak of the cosmic SFRD, at $z=1.86$, the uniform and normal priors underestimate the SFRD by a factor of 1.82 and 1.53 respectively, while the N-R underestimates it by a factor of 3.6. In the paper, @haslbauerCosmologicalStarFormation2023, they estimate a factor of 2.16 at the peak, using the SFR's from the UNGC catalogs. However, the ratio of the MCMC grows with $z$, since many galaxies start forming stars at a lookback time of $\sim 12$ Gyr, as seen in @fig-tsf-hist-both (the mean value of the $t_{sf}$ of the galaxies is 10 Gyr for the uniform prior and 9.8 Gyr for the normal prior), resulting in extreme deviations from the cosmic SFRD.

{{<pagebreak>}}

# Conclusions

With the use of IR data for the SFR of the LCV and with the use of different algorithms, we were able to reduce the divergence of the SFRD of the LCV from the cosmic SFRD from a factor of 2.16 to 1.53 at $z=1.86$. The N-R, can describe better the SFRD of the LCV compared to the MCMC for higher redshifts, but fails to approximate it for lower redshifts. The high divergence of the MCMC approximations, for $z>2$ is a result of the wrong prior of the $t_{sf}$. Using uniform priors for the time of star formations, severely underestimates the time in which most galaxies start the star forming process, meaning that most of the galaxies stop contributing at the SFRD at earlier lookback times than they should.

A better assumption for the prior of $t_{sf}$, which pushes the mean $\overline{t_{start}}\approx0.2$ Gyr, could correct the model at higher redshifts, since it would approximate the assumption of the N-R, while also keeping the lower factor of the MCMC at the peak of the Lilly-Madau plot. A possible prior for the $t_{sf}$ could be a skewed normal distribution with a mean value $\overline{t_{sf}}\approx 13$ Gyr and with a large sigma to cover the younger galaxies of the LCV. Another possible distribution could be a log-normal distribution for the $t_{start}$. But since the choice of a prior can significantly affect the results and because of the lack of a bibliography for this model, for this undergraduate thesis, the choice of a uniform is the safest choice.

Based on our results, we cannot safely assume that the model works for the galaxies of the LCV. Since parameterized SFH models do not account for bursty star formation, events characteristic of starburst galaxies, incorporating additional parameters who depend on $\text{SFR}_{H\alpha}$ and $\text{SFR}_\text{FUV}$ could increase the accuracy of the model. Additionaly, only examining one model ignores the intrinsic characteristics of the galaxies. @haslbauerCosmologicalStarFormation2023, introduce a combined SFH model depending on the mass of the galaxies, which can reduce the difference of the predicted model and the cosmic SFRD.

In summary, the use of the MCMC and the data from the HECATE catalog proves that the delayed-$\tau$ model a better approximation of the SFH, as originally argued by @haslbauerCosmologicalStarFormation2023. However, more fine-tuning of the model is needed to be certain if the delayed-$\tau$ model is a robust approximation of the SFH of the galaxies of the Local Cosmological Volume.
