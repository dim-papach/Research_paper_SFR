...
[^6]: $R^2$ Measures the proportion of variance explained by the linear model. For a model to be acceptable the $R^2$ should be atleast $80\%$
...
To begin with the morphological types of the galaxies we first need to understand possible incompatibilities of the given types of each catalog.

::: {#fig-TTypes layout-ncol="2"}
![Before the clipping of the transient types](compare/quickplots/Hubble%20Morphology.png)

![After the clipping of the transient types](compare/quickplots/Type_clip.png)

Scatter plots and the residuals of the Hubble Numerical morphological types show a good correlation between the two datasets, if we ignore the transient galaxies.
:::
