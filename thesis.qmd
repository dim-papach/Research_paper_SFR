...
::: {#fig-comp-sfr-obs-pred layout-ncol="2"}
![MCMC run for $\tau \sim \text{uniform}$ prior.](r_mcmc_uni/plots/sfr_diff_plot.png){#fig-comp-sfr-obs-pred-uni}

![MCMC run $\tau \sim \text{normal}$ prior.](r_mcmc_normal/plots/sfr_diff_plot.png){#fig-comp-sfr-obs-pred-norm}

Comparison of the observed SFR with the predicted by the parameters given by the MCMC run.
:::

::: {#fig-A-M-mcmc layout-ncol="2"}
...
Linear regression for $A_{del}-M_*$ for the $A_{del}$ values from the two MCMC runs. The black dashed line represents the regresion weighted by $x$. For the left plot, the regression yields a correlation of 94%, while the weighted regression is the fit $\log(A_{del}) = 0.95\cdot\log(M_*)+0.60$, with a correlation of 98%. For the right plot, the correlation is higher, at 99%, with a weighted regression of $\log(A_{del}) = 0.97\cdot\log(M_*)+0.44$, with a correlation of 99.5%.
:::
