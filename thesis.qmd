...
-   Distances \[Mpc\] (288/288 galaxies)
-   Radial Velocities \[km/s\] (286/288 galaxies)
-   Morphology and Geometry of the galaxies
    -   Morphological type (229/288 galaxies)
    -   Inclination \[degrees\] (209/288 galaxies)
    -   Angular Diameter of the Major and Semi-major axis \[arcmin\] (261/288 galaxies)
-   K-band Luminosity \[$L_\odot$\] and Magnitude (70/288 galaxies)
-   B-band Magnitudes \[mag\] (244/288 galaxies)
-   Star Formation Rates \[$M_\odot/yr$\] (73/288 galaxies)

For the comparison of our data we will use linear regression, accounting for the errors of the observed data, where $R^2$ and the slope of the fitted line should be close to 1[^6]. We will also plot the residuals ($y_\text{Regression}-y_\text{observed}$) and if the mean value $\overline{\text{Residuals}}\approx 0$, then the linear regression is succesfull.
...
All the results are shown in @sec-regressions, except the comparisons of the Morphological Types and of the SFRs,who need more detailed analysis.

To begin with the morphological types of the galaxies, we first need to address possible potential inconsistencies in the classifications of the two catalogs. As we can see in @fig-TType-all, some galaxies "jump" from one side of the classification to the other side, depending on the catalog. These galaxies are dwarf galaxies, so we can safely assume that they are Tr type galaxies, as discussed in @sec-dwarves. For the rest of the analysis we will adopt the Numerical Morphological Type from the HECATE catalog, since it includes a larger number of galaxies, ensuring better data compatibility. Additionally, the HECATE catalog boasts an accuracy of 0.1 and provides errors in comparison with the UNGC.
