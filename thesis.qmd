...
![Paramater distribution of the delayed-$\tau$ SFH for the galaxies of the LCV. The colourbar represents the present-day stellar mass of the galaxies. The left plot shows the whole parameter range of the model, including negative timescales and $A_{del}$ values that occur by instabilities. In the right plot, the region $\tau \in [0,16]$ and $A_{del}\in[0,10^{13}]$ is depicted. The majority of the galaxies have a timescale of about 3.5-4.5 Gyr as expected from @speagleHighlyConsistentFramework2014](NR/tau_A_double_plot.png){#fig-A-tau}
...
![$A_{del}-\text{sSFR}$ plot, where $A_{del}$ is calculated with different methods. In blue and green are the normalization constant values as they calculated by the MCMC method with different priors, while in red are results from Newton-Raphson.](method_comparison/sSFR_A.png){#fig-sSFR-A}
...
![Newton-Raphson Method](/NR/A-M_*-c_x.png){#fig-A-M}

Linear regression for $A_{del}-M_*$ for the $A_{del}$ values from the two MCMC runs and the Newton-Raphson method, with the colorbar repressenting the ratio $x=t_{sf}/\tau$. For the MCMC runs, the black dashed line represents the regresion weighted by $x$. For the upper left plot, the regression yields a correlation of 94%, while the weighted regression is the fit $\log(A_{del}) = 0.95\cdot\log(M_*)+0.60$, with a correlation of 98%. For the upper right plot, the correlation is higher, at 99%, with a weighted regression of $\log(A_{del}) = 0.97\cdot\log(M_*)+0.44$, with a correlation of 99.5%. For the N-R plot, we do two different regressions for the two distinct regions of $A_{del}$, with the upper region only having a correlation of 55\%, while the lower region closely relating the normalization constant with the Stellar Masses of the galaxies, as described in the MCMC plots.
:::

In @fig-sSFR-A, it can be seen that the MCMC run does not have the same inconsistancies with the NR method, and @fig-tau-hist-both shows that the MCMC method captures better both the galaxies with small timescales, and the galaxies with slow rising SFR's.

::: {#fig-distr-comparison layout-ncol="2"}
...
Distribution of the results of $\tau$ and $t_{sf}$ from all the methods. In blue are the distributions as they are calculated by the MCMC method with the uniform $\tau$ prior, in green are the results from the MCMC run with the normal prior, while in red are results from Newton-Raphson.
:::
...
Having calculated the model parameters, it is possible to recunstruct the SFH of the galaxies, and from the @eq-SFRD, it is possible to calculate the SFRD of the LCV and recreate the Lilly-Madau Plot. As seen from @fig-lilly-madau, the results from the MCMC method are more consistent with the theoretical line give by the [@eq-lilly-madau; @madauCosmicStarFormation2014], while the NR method underastimates the SFRD of the LCV, by a factor[^11] greater than $\sim 3$, up until $z \approx 3$. From the three methods, the MCMC with the prior $\tau\sim \text{normal}$ is systematicaly closer to universal SFRD, indicating that the assumption for the prior is better than the uniform prior, while the NR line can not approximate the expected line. Eventhough the MCMC method could approximate the SFRD for small redshifts, at greater $z$ the model can overestimate the SFRD by a factor of $\sim 2-3.2$.

[^11]: The factor is calculated by the residuals, $\log(SFRD_\text{data})-\log(SFRD_{LM})=\log\left(\frac{SFRD_\text{data}}{SFRD_{LM}}\right)\Leftrightarrow \frac{SFRD_\text{data}}{SFRD_{LM}} = 10^{\log(SFRD_\text{data})-\log(SFRD_{LM})}$

::: {#fig-lilly-madau layout-ncol="2"}
