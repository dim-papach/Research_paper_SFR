...
3.  [Pearson Correlation $\rho$]{.underline}: Measures the strength and direction of the linear relationship between two variables, ranging from -1 to 1.

In simple linear regression, $R^2$ is the square of the Pearson correlation coefficient $r$.
...
def scatter_plot(x, y, xerr = None, yerr = None):
    ...
    if xerr is None:
        xerr_d = np.ma.zeros(len(x_data))
    else:
        xerr_d = np.ma.array(xerr.value, mask=np.isnan(xerr))
    if yerr is None:
        yerr_d = np.ma.zeros(len(y_data))
    else:
