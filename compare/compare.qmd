...
```
```{python}
sns.distplot(dt["diff_T"])
plt.xlabel("Percentage change of Types of Galaxies")
plt.show()
```
...
```
```{python}
sns.distplot(dt["diff_inc"])
plt.xlabel("Percentage change of the inclination of the galaxies [%]")
plt.show()
```
...
```{python}
dt["log_a26"] = np.log(dt["a26_1"].data)
dt["log_a26"].unit = u.arcmin
dt["log_R1"] = np.log(dt["R1"].data)
