...
```{python}
def compare_data(x, y, unc=None, sigma = False):
    """
...
```{python}
def scatter_plot(x, y, xerr = None, yerr = None, sigma = False):
    """
    ...
    if outliers is not None:
        ...
        plt.scatter(x, filtered_data,  label="Fitted Data", c = "blue", fillstyle = "filled")
        plt.errorbar(x, y, yerr=yunc, fillstyle="filled", c = "black", label="Clipped Data", fmt = "o")
    else:
...
a26_corr = round(compare_data(dt["a26_1"], dt["A26_1a"], sigma =True)[3], 3)
```
...
| a26_1 (Major) | A26_1a (Semi-major axis) |                 angular diameter (arcmin)                 |     `{python} a26_corr`      |
...
scatter_plot(dt["inc"], dt["INCL"], yerr = dt["E_V"], sigma =True)
...
```{python}
#| echo: false
mag_B_corr = round(compare_data(dt["mag_B"], dt["BT"],unc = dt["E_BT"], sigma = False)[3], 3)
...
# Create a pair grid
sns.pairplot(sfr_data)
plt.show()

```

It is possible we dont see a correlation because we dont have a big enough common sample
