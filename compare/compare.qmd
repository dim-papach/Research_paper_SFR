...
```{python}
dt["diff_T"] = relative_diff(dt["T"], dt["TType"])
dt["diff_T"].info("stats")
```

```{python}
sigma_T = sigma_clip(dt["diff_T"])
sigma_T.info("stats"), dt["diff_T"].info("stats")
```

```{python}
sns.histplot(dt["diff_T"], kde = True, label="Unclipped data")
sns.histplot(sigma_T, kde = True, label = "Sigma=3")
...
```{python}
dt["diff_inc"] = relative_diff(dt["INCL"], dt["inc"])
dt["diff_inc"].info("stats") 
```

```{python}
sns.histplot(dt["diff_inc"], kde = True)
plt.title("Percentage change Distribution [%]")
plt.xlabel(f"Inclination of the galaxies [{dt['inc'].unit}]")
plt.show()
```
