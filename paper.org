...
morphology_hist("TType", "Morphology type code")
#+end_src
...
#plt.xscale("log")
plt.axvline((np.mean(dc["logSFR_ratio"].value)), color = "black", linestyle = "--", alpha = 0.8)
#text next to mean line "Mean = mean.value"
plt.text(np.log10(np.mean(dc["logSFR_ratio"].value))+1.3, plt.ylim()[1]-4, r"Mean of ratio = {:.2f}".format(np.mean(dc["SFR_ratio"])), color = "blue")
...
dc["tau_g"] = dc["M_g"]/dc["SFR_0"].to(u.solMass/u.Gyr)
dc["tau_g"].info("stats")
#+end_src
...
dc["x_n"] = x2
dc["A_n"] =MaskedColumn(dc["SFR_0"]*t_sf.to(u.yr)*np.exp(dc["x_n"])/(dc["x_n"]**2))
# A_n = nan where inf
...
: ---- ------------------- ------------------- ------------------- -------------------
:  x_n             1.66518             2.91531            -29.6974             11.9164
:  A_n 5.27714e+10 solMass 5.20772e+11 solMass 1.60997e-08 solMass 8.60539e+12 solMass
: None
...
#+RESULTS:
: /tmp/babel-7hf2gn/python-AkP2wD:23: RuntimeWarning: The iteration is not making good progress, as measured by the
...
: ---- ------- ------- -------------- -------------- ------------------- ------------------ -------------
:  x_f float64           MaskedColumn         1.5824             2.94119           -29.6974       9.79911
:  A_f float64 solMass MaskedQuantity 658979 solMass 8.09096e+06 solMass -0.0496767 solMass 1e+08 solMass
: None
...
---- ------- ------------ ------- ------- -------- -------
 x_f float64 MaskedColumn  1.5824 2.94119 -29.6974 9.79911
 x_n float64       Column 1.66518 2.91531 -29.6974 11.9164
None
:end:
...
: ---- ------- ------- -------------- ------------------- ------------------- ------------------- -------------------
:  A_f float64 solMass MaskedQuantity      658979 solMass 8.09096e+06 solMass  -0.0496767 solMass       1e+08 solMass
:  A_n float64 solMass MaskedQuantity 5.27714e+10 solMass 5.20772e+11 solMass 1.60997e-08 solMass 8.60539e+12 solMass
: None
...
#+begin_src python :results file value
plt.close("all")
sns.displot(np.log10(dc["tau"].value))
#mean line
#plt.axvline(np.log10(6.3), color = "black", linestyle = "--", alpha = 0.8)
#mean line
plt.axvline(np.log10(np.mean(dc["tau"].value)), color = "black", linestyle = "--", alpha = 0.8)
#text next to mean line "Mean = mean.value"
plt.text(np.log10(np.mean(dc["tau"].value))*1.1, plt.ylim()[1]-4, r"Mean = {:.2f}".format(np.mean(dc["tau"])), color = "blue")
...
#+RESULTS:
:results:
MAX of $A_{del}$: A0554+07
MIN of $A_{del}$: A0554+07
:end:
...
def func(x, a):
    return zeta * a*np.exp(12/x)/(np.exp(12/x)-1-12/x)

x_values = np.linspace(plt.xlim()[0], plt.xlim()[1], len(dc["StellarMass"]))
a_value = dc["StellarMass"].mean().value

for i in range(0,5):
    y_values = func(x_values, (a_value) * 0.2**i )
    plt.plot(x_values, y_values, label = r"$M_* = $"+ "{:.2e}".format((a_value+5) * 0.2**i))
...
plt.scatter( x=dc["x_n"], y=dc["SFR_0"], c = np.log10(dc["av_SFR_theor"].value), cmap = "viridis")
...
plt.xlabel(r"x")
plt.ylabel(r"$SFR_0$ [{}]".format(dc["SFR_0"].unit.to_string("latex")))
plt.tight_layout()
...
sns.pairplot(correlation_matrix, vars=["StellarMass", "MHI", "SFR_0", "av_SFR_theor", "M26"], kind="reg", diag_kind="kde")
plt.tight_layout()
plt.savefig("figure/pairplot")
plt.close("all")
print(correlation_matrix[["StellarMass", "MHI", "SFR_0", "av_SFR_theor", "M26", "M_g", "tau", "A_f"]].corr())
#+end_src

#+RESULTS:
:               StellarMass       MHI     SFR_0  av_SFR_theor       M26       M_g       tau       A_f
: StellarMass      1.000000  0.878513  0.893497      1.000000  0.991270  0.878513 -0.229484  0.473412
: MHI              0.878513  1.000000  0.980795      0.878513  0.887238  1.000000 -0.245085  0.223906
: SFR_0            0.893497  0.980795  1.000000      0.893497  0.898983  0.980795 -0.255432  0.202507
: av_SFR_theor     1.000000  0.878513  0.893497      1.000000  0.991270  0.878513 -0.229484  0.473412
: M26              0.991270  0.887238  0.898983      0.991270  1.000000  0.887238 -0.238870  0.472547
: M_g              0.878513  1.000000  0.980795      0.878513  0.887238  1.000000 -0.245085  0.223906
: tau             -0.229484 -0.245085 -0.255432     -0.229484 -0.238870 -0.245085  1.000000 -0.103207
: A_f              0.473412  0.223906  0.202507      0.473412  0.472547  0.223906 -0.103207  1.000000
...
: MHI           1.000000  0.861085      0.784292  0.850949  1.000000 -0.050360  0.115054 -0.040938
: SFR_0         0.861085  1.000000      0.855756  0.854753  0.861085 -0.027812 -0.065974 -0.543286
: av_SFR_theor  0.784292  0.855756      1.000000  0.894117  0.784292 -0.369653  0.392335 -0.428340
: M26           0.850949  0.854753      0.894117  1.000000  0.850949 -0.227629  0.234974 -0.285532
: M_g           1.000000  0.861085      0.784292  0.850949  1.000000 -0.050360  0.115054 -0.040938
: tau          -0.050360 -0.027812     -0.369653 -0.227629 -0.050360  1.000000 -0.561142 -0.022606
: A_f           0.115054 -0.065974      0.392335  0.234974  0.115054 -0.561142  1.000000  0.257164
: tau_g        -0.040938 -0.543286     -0.428340 -0.285532 -0.040938 -0.022606  0.257164  1.000000
